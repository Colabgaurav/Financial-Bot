...
GROQ_API_KEY = "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"  # Replace with your actual API key
client = Groq(api_key=GROQ_API_KEY)

openai.api_key = "sk-MTVra4cjKciDKJynPu9bRrA7gDOf7kmW9OvR-ZsFCVT3BlbkFJ_F2ZQkTC2yLqU7q52bsmXFGahd1Butg_1qzze1MEEA"
...
def create_html_report(json_data: dict, overview:str, quant_summary: str, quant_sentiment: str, news_summary: str, news_sentiment: str):
    ...
    metrics = ['sales', 'ebitda', 'net_profit', 'eps', 'total_assets', 'total_liabilities', 'long_term_debt', 'total_equity']
    ...
    <html>
        ...
        <body>
            ...
            <table>
                ...
                <tr><th>Total Liabilities</th><td>{key_financials['total_liabilities']}</td></tr>
                <tr><th>Long Term Debt</th><td>{key_financials['long_term_debt']}</td></tr>
            ...
            <table class="horizontal-year">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        {" ".join([f"<td>{five_year_summary[year]['total_liabilities']}</td>" for year in five_year_summary.keys()])}
                    </tr>
                    <tr>
                        <th>Long Term Debt</th>
                        {" ".join([f"<td>{five_year_summary[year]['long_term_debt']}</td>" for year in five_year_summary.keys()])}
                    </tr>
                    <tr>
                        <th>Total Equity</th>
...
def perform_analysis(org: str):
    ...
    Generate a fundamental analysis report in the following format:
    5-year summary fundamentals (2019, 2020, 2021, 2022, 2023, 2024)
    Must Use the units specifically for total_assets, total_liabilities, long_term_debt, total_equity provided in the data (if {org} is indian company use crores if american company use millions):
    "CAGR": {{
        "Total_Assets": "...",
    ...
    structure_prompt = f'''Combine the {annual_report} , {balance_report}, {cagr_percent} , {cagr_remaining} and must use the units specifically for sales, ebitda, net_profit, eps, total_assets, total_liabilities, long_term_debt, total_equity provided in the data structurize according to following format:
{{
    "company_name": "{org}",
...
st.markdown(
    f"[DCF Calculator Website]({"https://www.youtube.com/"})",
